...
**Code for finding the number of total NAs and their distributions:**
```{r}
...
```
There are 2304 NAs.
2012-10-01, 2012-10-08, 2012-11-01, 2012-11-04, 2012-11-09, 2012-11-10, 2012-11-14 and 2012-11-30 equally have 288 NAs respectively. This shows that if a NA appears on any interval of a day, then other intervals will be NAs, too.

The main idea of the strategy is to find the weakday of each day with all NAs, and then replace those NAs with the average of each interval on that weekday. For example, 2012-10-01 is a Saturday and the steps on that day are all NAs. Then I will replace the NAs on 2012-10-01 with average steps of Monday.
...
```
1. The avg.wids serves like a index here, recording the average steps on every weekday
2. Copy rawds to rawds2
3. Replace all NAs in the raw data with information in avg.wids.

**Code for calculating the average after replacing all NAs and its plot:**
```{r}
...
r
```
![r](instructions_fig/r.png)

**Code for calculating the mean and median:**
```{r}
mean(dds2$TotalSteps,na.rm=T)
median(dds2$TotalSteps,na.rm=T)
```

As calculated above, the mean is 10821.21 and the median is 11015. The average after filling the NAs grows higher than before.
...
2. Devide rawds2 into groups according to the factor just created and interval.
